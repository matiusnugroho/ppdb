...
<template>
	<div class="relative z-50">
		<label class="mb-3 block text-sm font-medium text-black dark:text-white"> Multiselect Dropdown </label>
		<div>
				...
				<div class="relative z-20 inline-block w-full">
					<div class="relative flex flex-col items-center">
							...
							<div
								...
								<div class="flex flex-auto flex-wrap gap-3">
									...
									<div v-show="selected.length === 0" class="flex-1">
										<input placeholder="Select an option" class="h-full w-full appearance-none bg-transparent p-1 px-2 outline-none" :value="selectedValues()" />
									</div>
								</div>
								<div class="flex w-8 items-center py-1 pl-1 pr-1">
									<button @click="open" type="button" class="h-6 w-6 cursor-pointer outline-none focus:outline-none" :class="isOpen() ? 'rotate-180' : ''">
						...
						<div class="w-full px-4">
							<div v-show="isOpen()" ref="target" class="max-h-select absolute left-0 top-full z-40 w-full overflow-y-auto rounded bg-white shadow dark:bg-form-input">
								<div class="flex w-full flex-col">
									<div v-for="(option, index) in options" :key="index">
										<div @click="select(index, $event)" class="w-full cursor-pointer rounded-t border-b border-stroke hover:bg-primary/5 dark:border-form-strokedark">
											<div :class="['relative flex w-full items-center border-l-2 border-transparent p-2 pl-2', option.selected ? 'border-primary' : '']">
